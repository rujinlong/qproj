...
#'
#' The directory name and the R package name are decoupled: `name`
#' defaults to the directory name, but can be set explicitly when the
#' directory follows a naming convention that `DESCRIPTION`'s `Package`
#' field cannot express (e.g. `pf116-DFG-vSCFA`). `Package` then takes
#' `name`, while `README.md` and the `.Rproj` file keep the directory
#' name.
#'
...
#'   `TRUE`.
#' @param name `character` R package name written to `DESCRIPTION`'s
#'   `Package` field. Must be a valid R package name (ASCII letters,
#'   digits and dots; starts with a letter; does not end with a dot).
...
#'   # directory name that is not a valid package name
#'   proj_create("../pf116-DFG-vSCFA", name = "pf116DFGvSCFA")
#' }
#' @export
#'
proj_create <- function(path, fields = list(), rstudio = TRUE, name = basename(path)) {

  if (length(fields) > 0 && (is.null(names(fields)) || any(names(fields) == ""))) {
    cli::cli_abort("`fields` must be a named list.")
  }

  # The directory name and the package name are two different things. `proj_name`
  # is what humans and git see; `pkg_name` is what goes into DESCRIPTION. They
  # coincide by default, so existing single-argument calls are unaffected.
  ...
  if (!is_valid_pkg_name(pkg_name)) {
    if (missing(name)) {
      cli::cli_abort(c(
        ...
        "i" = "To keep this directory name, pass a valid package name via {.arg name}.",
        "i" = "qproj reuses {.code DESCRIPTION} from R package conventions to manage dependencies."
      ))
    }
    cli::cli_abort(c(
      "{.arg name} {.val {pkg_name}} is not a valid R package name.",
      "i" = "Use only ASCII letters, digits, and dots; start with a letter; do not end with a dot.",
      "i" = "qproj reuses {.code DESCRIPTION} from R package conventions to manage dependencies."
    ))
  }
